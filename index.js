...
app.post("/webhook/byl", async (req, res) => {
  ...
  // A paid invoice could be either a new eSIM purchase or a top-up on an
  // existing one — check top-ups first since that table is more specific.
  const topupOrder = await db.getTopupOrderByInvoiceId(invoice.id);
  if (topupOrder) {
    await handleTopupPaid(topupOrder, invoice.id);
    return;
  }

  const convo = await db.getConversationByInvoiceId(invoice.id);
  if (!convo) {
    console.warn("No conversation or topup order found for paid invoice", invoice.id);
    return;
  }
...
async function handleTopupPaid(topupOrder, invoiceId) {
  try {
    await db.markTopupOrderPaid(invoiceId);

    // Re-fetch the current live price right before calling topup — eSIM
    // Access rejects stale prices, same lesson learned from ordering.
    const packages = await esimaccess.listTopupPackages(topupOrder.iccid);
    const pkg = packages.find((p) => p.packageCode === topupOrder.package_code);
    if (!pkg) {
      throw new Error(`Topup package ${topupOrder.package_code} no longer available for iccid ${topupOrder.iccid}`);
    }

    const result = await esimaccess.topupEsim({
      iccid: topupOrder.iccid,
      packageCode: topupOrder.package_code,
      transactionId: `topup_${invoiceId}_${Date.now()}`,
      amount: Number(pkg.price),
    });

    await msg.sendText(
      topupOrder.sender_id,
      `Дата амжилттай нэмэгдлээ! 🎉\nЗахиалга: ${topupOrder.order_no}\nНэмэгдсэн: ${topupOrder.gb} GB\nШинэ дуусах хугацаа: ${result.expiredTime || "—"}`
    );
  } catch (err) {
    console.error("Topup failed:", err.response?.data || err.message);
    await msg.sendText(
      topupOrder.sender_id,
      "Төлбөр амжилттай хийгдсэн ч дата нэмэхэд алдаа гарлаа. Манай тусламжийн баг тантай удахгүй холбогдоно."
    );
  }
}
